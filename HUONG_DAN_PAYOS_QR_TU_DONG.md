# HVHN — Cài thanh toán QR tự động PayOS (giá cấu hình → Discord)
...
Khách điền Form → nhận email lịch sự chứa **mã QR riêng** và nút mở trang PayOS → thanh toán đúng **giá đang cấu hình** (mặc định **99.999đ**) → PayOS gửi webhook đã ký → Apps Script kiểm tra chữ ký, `orderCode` và số tiền của chính đơn đó → bot tạo link Discord một lần → khách nhận email link Discord.
...
- Giá mặc định là `99.999đ`; quản lý có thể đổi an toàn bằng menu **Đổi giá gói học liệu** mà không sửa code.
...
- Kiểm tra đồng thời: chữ ký hợp lệ, `orderCode` thuộc đúng đơn và số tiền đúng với giá đã lưu trên dòng đơn.
...
4. Kiểm tra dòng **Giá gói hiện tại**. Muốn đổi giá cho các đơn tạo từ thời điểm đó trở đi, dùng **HVHN → 💳 Thanh toán tự động → 💰 Đổi giá gói học liệu**. Đơn đã tạo vẫn giữ nguyên giá cũ để webhook đối soát đúng.
...
2. Kiểm tra email:
   - Tiêu đề phải là **[HVHN] Mã QR thanh toán 99.999đ** (hoặc đúng giá bạn vừa cấu hình).
   - Có QR, mã tham chiếu `HVxxxxxxx`, nút **Mở trang thanh toán an toàn**.
   - Mở nút đó: trang PayOS phải hiện đúng giá đang cấu hình và QR.
3. Thực hiện một giao dịch test đúng số tiền trên QR vừa nhận. Không sửa nội dung chuyển khoản; kể cả bị ngân hàng chèn mã, luồng vẫn phải nhận được.
...
- Webhook PayOS không cấp quyền chỉ vì khách mở trang “thành công”; chỉ webhook có HMAC hợp lệ, đúng `orderCode` và đúng số tiền đã lưu của đơn mới cấp Discord.
...
điền Form, hệ thống gửi một link Discord riêng đến email của họ. Khi vào Discord, họ bấm **Kích hoạt
quyền truy cập tài liệu** và watcher vẫn cấp học liệu như luồng thông thường.
...
3. Menu **1. Kết nối sheet responses cũ** đã có sẵn nguồn mặc định:
   `1YB9tc7mHoLijcSJgUwSGiE1z-kQwTSPF03An92sgw68`, tab `gid=368858728`.
4. Nếu đổi sang sheet responses khác, bấm mục này và dán URL/ID sheet mới; có thể để trống tên tab nếu dùng `gid`
   trong URL. Cột email mặc định được tìm chính xác theo tiêu đề `Gmail của bạn là gì?`.
   - Allowlist được đọc trực tiếp từ sheet responses cũ, không dán thủ công nữa.
   - Sheet responses cũ độc lập với Sheet Phân phối; tab `_khach_preorder` chỉ là nhật ký cấp invite.
   - Email không nằm trong sheet responses cũ sẽ không nhận invite, kể cả khi họ có link Form.
...
1. Gửi cùng một link Form đó cho toàn bộ khách đã có trong sheet responses cũ.
2. Khách điền **Họ và tên** và **Email nhận link Discord**. Email phải trùng email trong sheet responses cũ
   (không phân biệt chữ hoa/thường).
3. Mỗi email trong sheet responses cũ chỉ được **submit Form một lần duy nhất**. Lượt submit thứ hai bị từ chối, kể cả
...
Nếu sheet responses cũ chưa có email nào hoặc hệ thống không tìm thấy đúng cột `Gmail của bạn là gì?`, tất cả lượt gửi sẽ bị từ chối an toàn.
Luồng pre-order chỉ cần `PMT_BOT_URL`, `PMT_SECRET` và `PMT_DAYS` đã cấu hình cho bot; không cần khóa PayOS hay webhook.
