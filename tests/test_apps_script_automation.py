...
class AppsScriptAutomationTest(unittest.TestCase):
    ...
    def test_customer_emails_hide_provider_names_and_attach_onboarding_materials(self):
        ...
        self.assertIn("function _guideOnlyCustomerOnboardingMaterials(materials)", self.src)
        self.assertIn("sent_with_guide_only_after_attachment_fallback", self.src)
        self.assertIn("CUSTOMER_MAIL_RELEASE", self.src)
    ...
    def test_preorder_form_persists_first_and_supports_an_explicit_resubmission(self):
        ...
        self.assertIn("const delivery = xuLyDonPreorderTuDong() || { status: 'scheduled' };", self.src)
